...
const FormFieldSchema = z.object({
  ...
  variableSuggestion: z.string().optional().describe("Variable système suggérée (TENANT_NAME, TENANT_NIF, TVA_AMT, etc.)."),
});
...
export async function analyzeFormLayout(input: { fileDataUri: string, documentTitle: string }): Promise<FormLayoutAnalysisOutput> {
  const { output } = await ai.generate({
    ...
    prompt: [
      ...
      Tente de mapper les champs sur ces variables connues : TENANT_NAME, TENANT_NIF, TENANT_ADDRESS, TOTAL_TVA, IRG_AMT, IBS_AMT, STAMP_DUTY.` }
    ],
    output: { schema: FormLayoutAnalysisOutputSchema },
    system: "Tu es un expert en traitement de documents administratifs. Tu extrais la structure géométrique des formulaires pour permettre leur remplissage automatique. Si le document est un PDF, analyse la première page pour définir la structure."
  });
