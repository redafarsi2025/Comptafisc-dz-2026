 ...
 * Intégration de la comptabilisation automatique et comparaison constructeur.
 */
...
import { 
  ...
  ArrowRight, CreditCard, Droplets, CheckCircle2, Calculator, AlertTriangle
} from "lucide-react"
...
export default function FuelManagementPage() {
  ...
  // Création d'une map des recommandations constructeurs pour comparaison rapide
  const vehicleRefs = React.useMemo(() => {
    if (!vehicles) return {};
    return vehicles.reduce((acc, v) => ({ ...acc, [v.id]: v.recommendedConsumption || 0 }), {});
  }, [vehicles]);
          ...
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow className="text-[9px] uppercase font-black px-6">
                ...
                <TableHead className="text-center">Efficience / Référence</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                const recommended = vehicleRefs[log.vehicleId] || 0;
                const isOver = recommended > 0 && log.efficiency > recommended * 1.15;
                      ...
                      {log.efficiency ? (
                        <div className="flex flex-col items-center">
                          <div className="flex items-center gap-1">
                            <span className={cn("text-xs font-black", isOver ? "text-red-500" : "text-emerald-600")}>
                              {log.efficiency.toFixed(1)}
                            </span>
                            {isOver && <AlertTriangle className="h-3 w-3 text-red-500 animate-pulse" />}
                          </div>
                          <span className="text-[7px] font-black uppercase opacity-40">
                             L/100KM {recommended > 0 && `(V.C: ${recommended})`}
                          </span>
                        </div>
                      ) : (
        ...
        <div className="text-xs leading-relaxed space-y-2">
          ...
          <div className="grid md:grid-cols-2 gap-8 mt-4">
             <div className="space-y-2 border-l-2 border-accent/20 pl-4">
                <p className="font-bold text-white uppercase text-[10px]">Comparaison Constructeur</p>
                <p className="opacity-70 leading-relaxed italic">
                  L'efficience est comparée à la valeur constructeur (V.C) saisie sur la fiche véhicule. Une alerte <AlertTriangle className="h-2 w-2 inline" /> apparaît si la consommation réelle dépasse de plus de 15% la norme théorique.
                </p>
             </div>
